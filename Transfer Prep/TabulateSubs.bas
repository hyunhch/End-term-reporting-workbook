...
'Can specify if we're looking for race, gender, or grade
    ...
    MajorArray = Array("Science", "Technology", "Engineering", "Math", "Undecided", "Other")
...
'Also called when roster is pulled
    ...
    TempArray = DemoTabulate(LRow - TableStart.Row, NameRange, "Major")
...
'Tabulate demographics for the indicated range and push to the Report Page
    ...
    Set StartRange = ReportSheet.Range("1:1").Find("Science", , xlValues, xlWhole)
    Set EndRange = ReportSheet.Range("1:1").Find("Other Major", , xlValues, xlWhole)
    Set MajorRange = ReportSheet.Range(StartRange.Address, EndRange.Address).Offset(PracticeRange.Row - 1, 0)
    ...
    TempArray = DemoTabulate(TotalNumber, SearchRange.Offset(0, 1), "Major")
    MajorRange = TempArray
    Erase TempArray
...
NoStudents:
    ...
    Set AllRange = Union(RaceRange, GenderRange, GradeRange, MajorRange, TotalRange)
