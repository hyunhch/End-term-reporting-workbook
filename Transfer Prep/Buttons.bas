...
'Delete everything from the report sheet
    ...
    With ReportSheet
        Set TotalRange = .Range("C2:C20")
        Set RaceRange = .Range("E2:L20")
        Set GenderRange = .Range("N2:P20")
        Set GradeRange = .Range("R2:U20")
        Set MajorRange = .Range("W2:AA20")
        Set DescriptionRange = .Range("AB2:AB20")
    End With
                                                 
            
    Set DelRange = Union(TotalRange, RaceRange, GenderRange, GradeRange, MajorRange, DescriptionRange)
    'Set DelRange = Union(DelRange, GenderRange)
    'Set DelRange = Union(DelRange, GradeRange)
...
'Clears the totals row of the report
    ...
    Set StartRange = ReportSheet.Range("1:1").Find("<45", , xlValues, xlWhole)
    Set EndRange = ReportSheet.Range("1:1").Find("Other Credits", , xlValues, xlWhole)
    Set GradeRange = ReportSheet.Range(StartRange.Address, EndRange.Address).Offset(1, 0)
    
    Set StartRange = ReportSheet.Range("1:1").Find("Science", , xlValues, xlWhole)
    Set EndRange = ReportSheet.Range("1:1").Find("Other Major", , xlValues, xlWhole)
    Set MajorRange = ReportSheet.Range(StartRange.Address, EndRange.Address).Offset(1, 0)
    
    Set TotalRange = ReportSheet.Range("1:1").Find("Total", , xlValues, xlWhole).Offset(1, 0)
    
    'Delete
    Set DelRange = Union(RaceRange, GenderRange, GradeRange, MajorRange, TotalRange)
...
'Go through every practice and tabulate
    ...
    Set ReportSheet = Worksheets("Report Page")
    Set SavedSheet = Worksheets("Saved Activities")
    Set AllPractices = SavedSheet.Range("C1:T1")
